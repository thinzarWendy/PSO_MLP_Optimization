...
from SwarmConfig import SwarmConfig
...
class PSOMLP:
    ...
    def fit(self, x, y, iterations=100, nparticles=100, early_stop=None, min_err=-np.inf, print_step=True):
        ...
        n_out = y.shape[1] if len(y.shape) > 1 else 2
        layers = [n_in, *self.hlayers, n_out]
        print(layers)
        self.swarm = [ SwarmConfig([x, y], layers, self.ativation, self.c1, self.c2, self.w, self.alpha) for _ in range(nparticles)]
